...
print(p_2.6)
#For vertify the answer
v <- Matrix_p_2.6 %*% p_2.6
v
all.equal(as.vector(v),y_p_2.6)
```
...
```{r}
#| label: prob 2.7c
column_sum <- colSums(aMat)
pair_sum <- outer(column_sum,column_sum,"+")
pair_perm <- which (pair_sum>75,arr.ind = TRUE)
colnames(pair_perm) <- c("col1","col2")
pair_no_repetition <- which(pair_sum>75&upper.tri(pair_sum),arr.ind = TRUE)
colnames(pair_no_repetition) <- c("col1","col2")
pair_perm
pair_no_repetition
```
